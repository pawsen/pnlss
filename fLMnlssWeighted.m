function [model,y_mod,models,Cost] = fLMnlssWeighted(u,y,model,MaxCount,W,lambda,LambdaJump)
%FLMNLSSWEIGHTED Optimize PNLSS model using weighted Levenberg-Marquardt algorithm.
%
%	Usage:
%		[model,y_mod,models,Cost] = fLMnlssWeighted(u,y,model,MaxCount,W,lambda,LambdaJump)
...
%		parameters of a PNLSS model (i.e. the elements of the matrices A,
%		B, C, D, E, and F). The difference between the modeled and the
%		measured output is minimized in a weighted least squares sense,
%		either in the time or the frequency domain. A simple stabilization
%		method can be applied by simulating a validation data set during
%		estimation and checking whether or not the modeled output stays
%		within prespecified bounds. If not, the Levenberg-Marquardt
%		iteration acts as if the cost function increased.
%
%	Output parameters:
...
%		1.0 : October 5, 2015
%       1.1 : April 20, 2016
%           Current folder added to search path before moving to temp folder
%           Help updated
